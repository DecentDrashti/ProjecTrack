...
export default async function FacultyEditAnnouncement({ params }: { params: { id: string } }) {
  const id = parseInt(params.id);
  const a = await prisma.announcement.findUnique({
    where: { AnnouncementID: id }
  });

  if (!a) return <div className="p-20 text-center font-black">Broadcast not found</div>;
      ...
      <div className="relative z-10 w-full max-w-xl group">
        ...
        <div className="relative bg-white/40 backdrop-blur-[30px] rounded-[2.5rem] border border-white/60 shadow-2xl shadow-slate-200/50 p-8 md:p-12">
          <div className="mb-10 text-center">
            <h1 className="text-4xl font-black text-[#201E43] tracking-tighter mb-2 uppercase">Adjust Message</h1>
            <p className="text-[#201E43]/60 font-black italic">"Refine your broadcast and share it."</p>
          </div>
          ...
          <form action={UpdateAnnouncementAction} className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-[#201E43]/60 uppercase tracking-[0.2em] ml-1">Core Info</label>
              <textarea
                name="Message"
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-[#201E43]/60 uppercase tracking-[0.2em] ml-1">Target</label>
              <select
                name="TargetRole"
            ...
            <button
              type="submit"
              className="w-full py-5 bg-[#201E43] text-white rounded-[2rem] font-black uppercase tracking-[0.3em] text-[10px] hover:scale-[1.02] active:scale-[0.98] transition-all duration-500 shadow-xl shadow-[#201E43]/10"
            >
